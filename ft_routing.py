"""
 ...
 (license header omitted for brevity — keep original in submission)
"""
#!/usr/bin/env python3
...
class FTRouter(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Mininet names switches s1,s2,... → dpid = idx+1 (1-based, avoids dpid=0)
        self.dpid_to_ip = {}
        self.ip_to_dpid = {}
    ...
    def _install_ft_routing(self):
        ...
        for dpid, datapath in self.datapaths.items():
            ...
            # ---------------------------------------------------------- #
            # CORE: one /16 entry per pod → correct aggr switch (col)    #
            # ---------------------------------------------------------- #
            if role[0] == 'core':
            ...
            # ---------------------------------------------------------- #
            # AGGREGATION                                                 #
            # Priority 20: /24 per edge subnet in same pod               #
            ...
            # ---------------------------------------------------------- #
            # EDGE                                                        #
            # Priority 20: /24 for OWN subnet → drop to controller       #
            #   (host /32 entries at priority 30 handle actual delivery;  #
            #    this blocks the suffix rule from looping local traffic)  #
            # Priority 10: suffix /8 → aggr uplinks (inter-pod only)     #
            # ---------------------------------------------------------- #
            elif role[0] == 'edge':
                pod    = role[1]
                sw_idx = role[2]

                # Install /24 entry for own subnet at priority 20.
                # This sits ABOVE the suffix rule (10) but BELOW host entries (30),
                # so local packets go to controller until /32 is learned,
                # and the suffix rule never matches local destinations.
                match = parser.OFPMatch(
                    eth_type=ether_types.ETH_TYPE_IP,
                    ipv4_dst=(f"10.{pod}.{sw_idx}.0", "255.255.255.0"))
                self.add_flow(datapath, 20, match,
                              [parser.OFPActionOutput(
                                  datapath.ofproto.OFPP_CONTROLLER,
                                  datapath.ofproto.OFPCML_NO_BUFFER)])

                # Suffix entries for inter-pod traffic
    ...
    def _install_host_entry(self, datapath, host_ip, port):
        """Priority 30 — highest, overrides /24 and suffix rules."""
        parser  = datapath.ofproto_parser
        match   = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP,
                                  ipv4_dst=host_ip)
        self.add_flow(datapath, 30, match, [parser.OFPActionOutput(port)])
    ...
    def _send_arp_reply(self, datapath, port, dst_mac, src_mac, src_ip, dst_ip):
        ...
        out = parser.OFPPacketOut(
            ...
            actions=[parser.OFPActionOutput(port)], data=p.data)
    ...
    def _packet_in_handler(self, ev):
        ...
        # Learn source host → install /32 delivery entry
        if self._is_host_port(dpid, in_port):
            if dpid not in self.host_port:
                self.host_port[dpid] = {}
            if src_ip not in self.host_port[dpid]:
        ...
        # Forward this first packet using two-level logic
        k    = self.k
        half = k // 2
        ft   = self.topo_net
